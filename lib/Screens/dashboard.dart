...
class _DashboardScreenState extends State<DashboardScreen> with TickerProviderStateMixin {
  ...
  Future<void> _loadData() async {
    ...
    final patientsList = results[2] as List;
    ...
    setState(() {
      ...
      confirmedCount = allRecords.where((r) {
    ...
    print('>>> confirmedCount: $confirmedCount (invoiced/confirmed/validated today only)');
    ...
    print('>>> todayCount: $todayCount (invoiced/confirmed/validated only)');
      loading = false;
    });
    _animCtrl.forward();
  }
              ...
              child: SingleChildScrollView(
                ...
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ...
                  Row(children: [
                    ...
                    Expanded(
                      child: GestureDetector(
                        onTap: () => _showHistoryDialog('records', l10n, isRtl),
                        child: _statCard(l10n.t('medicalRecords'), todayStats['records'] ?? 0,
                  ...
                  Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                    ...
                    TextButton.icon(
                      onPressed: () {
                        NavigationHistory().addRoute('/records');
  ...
  void _showHistoryDialog(String type, AppLocalizations l10n, bool isRtl) async {
    ...
    if (type == 'consultations') {
      ...
      // Affichage par jour pour les dossiers médicaux facturés uniquement (30 derniers jours)
      for (int i = 0; i < 30; i++) {
        ...
        int count = dataList.where((r) {
          ...
          final dateConsult = r['date_consultation'].toString().split(' ')[0];
          final state = r['state']?.toString().toLowerCase() ?? '';
          return dateConsult == dateKey && (state == 'invoiced' || state == 'confirmed' || state == 'validated');
        }).length;
        
        history[dateKey] = count;
      }
      print('>>> records history (invoiced only): $history');
    } else {
    ...
    final title = type == 'patients' ? l10n.t('patientsHistory')
                 : type == 'records' ? l10n.t('recordsHistory')
              ...
              : ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    if (type == 'consultations' && dateParts.length == 2) {
                      ...
                      // Format jour pour patients et records
                    ...
                    return ListTile(
                      leading: Container(
                        ...
                        decoration: BoxDecoration(
                          color: type == 'patients' ? AppColors.primaryLight
                              : type == 'records' ? AppColors.purpleLight
                      ...
                      trailing: Text('$count ${type == 'patients' ? l10n.t('navPatients') : type == 'records' ? l10n.t('navRecords') : l10n.t('consultations')}',
                          style: isRtl
  ...
  Widget _tableRow(Map r, bool isLast, AppLocalizations l10n, bool isRtl) => Container(
    ...
    child: Row(children: [
      ...
      Expanded(child: Container(
        ...
        decoration: BoxDecoration(
          color: _getStatusBg(r['state']?.toString()),
          borderRadius: BorderRadius.circular(20),
          border: Border.all(color: _getStatusColor(r['state']?.toString()).withValues(alpha: 0.25)),
        ),
        child: Text(_getStatusLabel(r['state']?.toString(), l10n), textAlign: TextAlign.center,
            style: isRtl
                ? GoogleFonts.cairo(color: _getStatusColor(r['state']?.toString()), fontSize: 11, fontWeight: FontWeight.w600)
                : GoogleFonts.dmSans(color: _getStatusColor(r['state']?.toString()), fontSize: 11, fontWeight: FontWeight.w600)),
      )),
    ]),
  );

  String _getStatusLabel(String? state, AppLocalizations l10n) {
    final s = (state ?? 'draft').toLowerCase();
    
    if (s == 'invoiced') return l10n.t('statusInvoiced');
    if (s == 'validated') return l10n.t('statusConfirmed');
    if (s == 'confirmed') return l10n.t('statusConsulted');
    return l10n.t('statusWaiting');
  }

  Color _getStatusColor(String? state) {
    final s = (state ?? 'draft').toLowerCase();
    
    if (s == 'invoiced') return AppColors.primary;
    if (s == 'validated') return AppColors.green;
    if (s == 'confirmed') return AppColors.purple;
    return AppColors.yellow;
  }

  Color _getStatusBg(String? state) {
    final s = (state ?? 'draft').toLowerCase();
    
    if (s == 'invoiced') return AppColors.primaryLight;
    if (s == 'validated') return AppColors.greenLight;
    if (s == 'confirmed') return AppColors.purpleLight;
